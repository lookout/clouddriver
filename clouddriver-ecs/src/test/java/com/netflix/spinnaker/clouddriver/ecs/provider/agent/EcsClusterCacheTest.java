...
public class EcsClusterCacheTest extends CommonCachingAgent {
  ...
  public void shouldRetrieveFromWrittenCache() {
    ...
    when(providerCache.get(ECS_CLUSTERS.toString(), key)).thenReturn(cacheResult.getCacheResults().get(ECS_CLUSTERS.toString()).iterator().next());
    ...
    Assert.assertTrue("Expected cluster name to be " + CLUSTER_NAME_1 + " but got " + ecsCluster.getName(), CLUSTER_NAME_1.equals(ecsCluster.getName()));
    Assert.assertTrue("Expected cluster ARN to be " + CLUSTER_ARN_1 + " but got " + ecsCluster.getArn(), CLUSTER_ARN_1.equals(ecsCluster.getArn()));
    Assert.assertTrue("Expected cluster account to be " + ACCOUNT + " but got " + ecsCluster.getAccount(), ACCOUNT.equals(ecsCluster.getAccount()));
    Assert.assertTrue("Expected cluster region to be " + REGION + " but got " + ecsCluster.getRegion(), REGION.equals(ecsCluster.getRegion()));
  }
}

...
public class TaskDefinitionCacheTest extends CommonCachingAgent {
  ...
  public void shouldRetrieveFromWrittenCache() {
    ...
    when(providerCache.get(TASK_DEFINITIONS.toString(), key)).thenReturn(cacheResult.getCacheResults().get(TASK_DEFINITIONS.toString()).iterator().next());
    ...
    Assert.assertTrue("Expected the task definition to be " + taskDefinition + " but got " + retrievedTaskDefinition,
